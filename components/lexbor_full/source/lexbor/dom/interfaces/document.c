                      ...
                      lxb_dom_document_dtype_t type, unsigned int ns)
{
    ...
    /*
     * Arielo MiniPC 10Z safety patch:
     * Keep owner_document valid from the beginning.
     * On small embedded heaps lxb_dom_document_init() can fail halfway.
     * The original failure path later calls lxb_dom_document_destroy(doc);
     * with owner_document still NULL, which enters the "owned by another"
     * branch and dereferences owner->mraw, causing EXCVADDR 0x50.
     */
    node->owner_document = (owner != NULL) ? owner : document;
...
lxb_dom_document_destroy(lxb_dom_document_t *document)
{
    ...
    if (lxb_dom_interface_node(document)->owner_document != document) {
        ...
        /* Arielo MiniPC 10Z safety patch: if initialization failed before
         * owner_document was set, avoid dereferencing NULL owner. */
        if (owner == NULL) {
            lexbor_mraw_destroy(document->text, true);
            lexbor_mraw_destroy(document->mraw, true);
            lexbor_hash_destroy(document->tags, true);
