...
// 09A_FIX1_CELESTE_USB_FALLBACK:
// El componente breezy_bt ya define bt_keyboard_*.
// Para no duplicar símbolos, aquí hacemos que esas funciones puedan leer
// también el teclado USB HID real del MiniPC.
extern int usb_hid_keyboard_connected(void);
extern int usb_hid_keyboard_raw_is_pressed(uint8_t keycode);
extern uint8_t usb_hid_keyboard_raw_get_modifiers(void);



// 02D: delay de arranque antes de escanear teclado.
// Deja estabilizar LCD/VTerm/BreezyBox antes de iniciar BLE scan.
#define BT_KBD_BOOT_SCAN_DELAY_MS 2000
...
static void print_adv_name(const struct ble_hs_adv_fields *fields, char *out, size_t out_len)
{
    ...
    if (fields->name && fields->name_len > 0) {
        ...
        snprintf(out, out_len, "(sin nombre)");
    }
}

static void log_scan_device(const struct ble_gap_disc_desc *disc,
                            const struct ble_hs_adv_fields *fields,
                            int is_hid)
{
    char name[64];
    print_adv_name(fields, name, sizeof(name));

    int appearance = fields->appearance_is_present ? fields->appearance : -1;

    ESP_LOGI(TAG,
             "SCAN DEV: %02x:%02x:%02x:%02x:%02x:%02x type=%d rssi=%d name='%s' appearance=%d uuids16=%d is_hid=%d",
             disc->addr.val[5], disc->addr.val[4], disc->addr.val[3],
             disc->addr.val[2], disc->addr.val[1], disc->addr.val[0],
             disc->addr.type, disc->rssi, name, appearance,
             fields->num_uuids16, is_hid);
}
    ...
    case ESP_HIDH_OPEN_EVENT:
        if (p->open.status == ESP_OK) {
            ESP_LOGI(TAG, ">>> KEYBOARD CONNECTED (HID) <<< (uptime=%lu ms)",
                     (unsigned long)(xTaskGetTickCount() * portTICK_PERIOD_MS));
            printf("\r\n[02D_FIX1] TECLADO BLE CONECTADO\r\n");
            s_connected = 1;
        } else {
...
static void connect_task(void *arg) {
    ...
    while (1) {
        ...
        if (s_scan_requested) {
            ...
            if (!s_connected && !s_is_scanning) {
                if (s_have_target) {
                    ESP_LOGI(TAG, "02D: Starting scan to find saved keyboard...");
                    start_background_scan();
                } else {
                    ESP_LOGI(TAG, "02D: No saved keyboard - starting first-pairing scan...");
                    printf("\r\n[02D_FIX1] BLE keyboard: ponga el teclado en modo emparejamiento...\r\n");
                    start_general_scan();
                }
            }
            continue;
...
static int scan_event_cb(struct ble_gap_event *event, void *arg) {
    if (event->type == BLE_GAP_EVENT_DISC) {
        ...
        // 02D_FIX1_VERBOSE:
        // Mostrar todos los dispositivos BLE vistos, no solo HID.
        log_scan_device(&event->disc, &fields, is_hid);
...
found_device:
        ...
        printf("\r\n[02D_FIX1] Teclado BLE encontrado, conectando...\r\n");
        ...
        } else if (!s_connected && !s_have_target) {
            ESP_LOGW(TAG, "02D_FIX1: scan complete, no BLE HID keyboard found");
            printf("\r\n[02D_FIX1] Scan terminado: si no aparece su teclado en el log, probablemente es Bluetooth Classic, no BLE.\r\n");
        }
    }
...
static void start_general_scan(void) {
    ...
    struct ble_gap_disc_params p = {
        .passive = 0,              // active scan para intentar obtener nombre
        .filter_duplicates = 0,    // verbose: queremos ver repeticiones/RSSI
        .itvl = 0x80,
        .window = 0x60,
    };

    printf("\r\n[02D_FIX1] Escaneando BLE 30s. Si el teclado es BLE debe aparecer aquí.\r\n");

    if (ble_gap_disc(s_own_addr_type, 30000, &p, scan_event_cb, NULL) != 0) {
        s_is_scanning = 0;
    }
}
...
int bt_keyboard_connected(void) {
    // 09A_FIX1: para apps externas, un teclado USB HID también cuenta
    // como teclado disponible.
    return s_connected || usb_hid_keyboard_connected();
}
...
int bt_keyboard_is_pressed(uint8_t keycode) {
    ...
    // 09A_FIX3: si no viene por BLE, mirar el teclado USB HID.
    // Sin logs, para no ensuciar la terminal mientras se escribe.
    if (!result) {
        result = usb_hid_keyboard_raw_is_pressed(keycode);
    }
...
uint8_t bt_keyboard_get_modifiers(void) {
    ...
    // 09A_FIX1: combinar modificadores BLE + USB.
    result |= usb_hid_keyboard_raw_get_modifiers();
...
static void boot_timer_cb(TimerHandle_t xTimer) {
    if (!s_connect_task || s_connected) {
        return;
    }

    // 02D:
    // - Si ya hay teclado guardado, busca ese teclado.
    // - Si no hay teclado guardado, inicia scan general para primer emparejamiento.
    s_scan_requested = 1;
    xTaskNotifyGive(s_connect_task);
}
...
static void on_sync(void) {
    ...
    // 02D:
    // Siempre arrancamos el temporizador.
    // Si hay teclado guardado, hará reconnect.
    // Si no hay teclado guardado, hará scan general para primer emparejamiento.
    ESP_LOGI(TAG, "02D: Boot keyboard timer start (%d ms), have_target=%d",
             BT_KBD_BOOT_SCAN_DELAY_MS, s_have_target);
    if (s_boot_timer) {
...
esp_err_t bt_keyboard_init(void) {
    ...
    s_boot_timer = xTimerCreate("bt_boot", pdMS_TO_TICKS(BT_KBD_BOOT_SCAN_DELAY_MS), pdFALSE, NULL, boot_timer_cb);
