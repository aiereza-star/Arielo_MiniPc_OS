...
#define WAIT_FOR_READY_TIMEOUT_MS 15000
#define READ_CAPACITY_READY_RETRIES 12
#define READ_CAPACITY_READY_RETRY_MS 400
...
static esp_err_t msc_wait_for_ready_state(msc_device_t *dev, size_t timeout_ms)
{
    ...
    uint32_t trials = MAX(1, timeout_ms / 250);
    ...
    do {
        ...
        if (err == ESP_OK) {
            ...
            // Algunos pendrives reportan UNIT ATTENTION:
            // ASC 0x28 / ASCQ 0x00 = Not ready to ready change, medium may have changed.
            // Es normal tras conectar/reconectar. REQUEST SENSE lo limpia y se reintenta.
            MSC_RETURN_ON_ERROR( scsi_cmd_sense(dev, &sense) );

            if (sense.key == MSC_UNIT_ATTENTION && sense.code == 0x28 && sense.code_q == 0x00) {
                ESP_LOGW(TAG, "UNIT ATTENTION 28/00: medium changed, retry ready");
...
static esp_err_t msc_read_capacity_retry(msc_device_t *dev, uint32_t *block_size, uint32_t *block_count)
{
    esp_err_t err = ESP_FAIL;

    for (int i = 1; i <= READ_CAPACITY_READY_RETRIES; i++) {
        *block_size = 0;
        *block_count = 0;

        err = scsi_cmd_read_capacity(dev, block_size, block_count);
        if (err == ESP_OK && *block_size != 0 && *block_count != 0) {
            if (i > 1) {
                ESP_LOGI(TAG, "READ CAPACITY OK tras %d intentos", i);
            }
            return ESP_OK;
        }

        // Tras UNIT ATTENTION 28/00, algunos medios pasan TEST UNIT READY
        // pero READ CAPACITY todavia falla durante unos ciclos.
        // scsi_cmd_read_capacity() ya ejecuta REQUEST SENSE al fallar,
        // asi que aqui solo esperamos y reintentamos.
        ESP_LOGW(TAG, "READ CAPACITY retry %d/%d: %s",
                 i, READ_CAPACITY_READY_RETRIES, esp_err_to_name(err));
        vTaskDelay(pdMS_TO_TICKS(READ_CAPACITY_READY_RETRY_MS * i));
    }
...
esp_err_t msc_host_install_device(uint8_t device_address, msc_host_device_handle_t *msc_device_handle)
{
    ...
    MSC_GOTO_ON_ERROR( msc_read_capacity_retry(msc_device, &block_size, &block_count) );
