...
#define PAD_SPEED   3               // px por tick
...
static void new_game(void)
{
    ...
    s_ai_reaction = 2;   // 0..~4: cuanto "tarda" en reaccionar la IA
    new_ball(1);
}
...
int main(int argc, char **argv)
{
    ...
    while (running) {
        ...
        if (scored) {
            // La pelota "salto" de sitio (relanzada): mejor redibujar todo
            // el campo una vez que arriesgar a dejar un rastro de la bola
            // en su posicion anterior fuera de pantalla.
            draw_hud_score();
            render_full();
        } else {
